...
* **리뷰 언어:** Gemini Code Assistant는 모든 코드 리뷰 요약 및 코멘트를 반드시 **한국어(Korean)**로 작성해야 합니다. 한국인이 쉽게 이해할 수 있도록 명확하고 자연스러운 한글 설명을 사용해야 하며, 영어나 기타 언어 사용은 허용되지 않습니다.
* **디자인 원칙 검토:** 모든 PR에 대해 **SOLID 원칙**, **디미터의 법칙(Law of Demeter)**, **보이스카우트 법칙(Boy Scout Rule)** 위반 여부를 중점적으로 확인하고 피드백을 제공해야 합니다.

### 0.1. 핵심 디자인 원칙 상세 설명 (SOLID, LoD & Boy Scout Rule)
...
#### 0.1.3. 보이스카우트 법칙 (Boy Scout Rule)

* **핵심 지침:** **"코드를 처음 발견했을 때보다 더 깨끗하게 남겨두라."**
* **실천 방법:**
    * 코드를 수정하거나 읽을 때 주변 코드의 작은 개선 사항을 함께 수행합니다.
    * 불필요한 주석 제거, 명명 개선, 중복 코드 제거 등 간단한 리팩토링을 즉시 적용합니다.
    * 큰 리팩토링이 필요한 경우 별도의 작업으로 계획하되, 작은 개선은 즉시 반영합니다.
* **주의 사항:** 변경 범위가 너무 넓어지지 않도록 주의하며, 현재 작업과 관련 없는 파일까지 수정하지 않습니다.
...
*   **단일 책임 원칙 (SRP) - 함수:** 함수는 **단 하나의 역할**만 수행해야 합니다. 여러 작업을 수행하는 함수는 작은 단위로 분리하여 각 함수가 명확하고 독립적인 책임을 가지도록 설계합니다.
...
* **중괄호 스타일 (Bracing):** **"Allman" 스타일**을 사용합니다. 여는 중괄호와 닫는 중괄호는 자체 줄에 위치하며 현재 들여쓰기 수준에 맞춥니다. 단, `if` 문의 내용이 한 줄일 경우 중괄호를 생략하고 한 줄에 작성하는 것을 허용합니다.
